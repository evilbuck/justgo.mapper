...
function compare(routeA, routeB) {
  return q.all([
    ...
    .spread((a, b) => {
      ...
      var difference = Math.max.apply(null, times) - Math.min.apply(null, times)
      var recommendedRouteDirections;

      if (timeA > timeB) {
        recommendedRouteDirections = a;
      } else {
        recommendedRouteDirections = b;
      }

      var recommendedRoute = timeA > timeB ? routeB : routeA;

      return { recommendedRoute, difference, recommendedRouteDirections };
    });
}
